...
class CNN(nn.Module):
    ...
    def __init__(self,
        ...
        if len(convolution_layers) - 1 != len(kernel_sizes) or len(convolution_layers) - 1 != len(strides):
            raise ValueError("The number of convolution layers - 1 must be equal to the number of kernel sizes and strides")
        ...
        for input_size, hidden_output_size, kernel_size, stride in zip(convolution_layers, convolution_layers[1:], kernel_sizes, strides):
            # TODO: make padding as a parameter !
        ...
        elif len(hidden_activations) != len(convolution_layers) - 1:
